...
# =========================
# 3. BUILD LATEST REPAIR FEATURES
# =========================
...
latest_repair AS (
    SELECT rf.*
    FROM repair_features rf
    INNER JOIN (
        SELECT
            plate_no,
            MAX(last_repair_date) AS max_repair_date
        FROM repair_features
        GROUP BY plate_no
    ) latest
        ON rf.plate_no = latest.plate_no
       AND rf.last_repair_date = latest.max_repair_date
)
...
if df.empty:
    print("No repair records found. No predictions generated.")
    conn.close()
    exit()

print("Motorcycles with repair history:", len(df))
...
pred_days = np.round(pred_days).astype(int)
pred_days = np.clip(pred_days, 1, None)

df["days_until_next_repair"] = pred_days

df["last_repair_date"] = pd.to_datetime(df["last_repair_date"])
df["next_repair_date"] = df.apply(
    lambda row: row["last_repair_date"] + timedelta(days=int(row["days_until_next_repair"])),
    axis=1
)
...
with conn.cursor() as cursor:
    for _, row in df.iterrows():
        cursor.execute(insert_sql, (
            row["plate_no"],
            row["last_repair_date"].strftime("%Y-%m-%d %H:%M:%S"),
            int(row["days_until_next_repair"]),
            row["next_repair_date"].strftime("%Y-%m-%d"),
            MODEL_VERSION
        ))
        success_count += 1
...
print(f"Repair predictions stored successfully: {success_count}")